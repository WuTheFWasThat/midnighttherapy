" Library for obtaining pathery.com statistics"
# pylint: disable=invalid-name, missing-docstring, bare-except, redefined-outer-name, dangerous-default-value
# pylint: disable=too-many-locals, unidiomatic-typecheck, too-many-branches
import datetime
import json
...
def user_string_from_id(userid):
    return id_user_map.get(id, 'anon:%s' % str(userid))
...
def get_mapcodes_of_type(maptype=None):
    ...
    while mapid > 0:
        ...
        if info:
            if (maptype is None) or (str(info[u'name']) == maptype):
                print info[u'code']
        mapid -= 1
        ...
        if user_score is None:
            ...
            if user_score == max_score:
                ...
                if tied_streak_start is not None:
                    print 'tied:', tied_streak_start, '-', mapid, \
                          '#maps: ', (mapid - tied_streak_start + 1)
                    tied_streak_start = None
                print 'unmaxed:', mapid, map_type, user_score, '<', max_score

        mapid -= 1

def get_extremal_total_days():
...
def find_sweep_stoppers(user=None):
    ...
    while True:
        ...
        if counted[0][0] == 3:
            ...
            if winner == user or user is None:
                print winner.ljust(20), 'got 3 wins on day ', isodate(date), \
                      ' (stopped by ', stopper, ')'
        date -= datetime.timedelta(days=1)
...
# 3/3 was snap's join date, 3/11 starts having regular maps, 3/16-3/29 is broken
first_pathery_date = datetime.datetime(2011, 3, 30)
...
def print_user_history(user, options={}):
    ...
    while True:
        ...
        def str_frac(num, den):
            return "%s/%s = %s%%" % (
                str(num), str(den), "NaN" if den == 0 else str((0.0 + num)/den)
            )
        ...
        if score is not None:
            ...
            if maxscore == score:
                ...
                if find_max_user(mapid) == userid:
                    ...
                    print ' | '.join([
                        x.ljust(8) for x in
                        ['Mapid', 'Points', 'Moves', 'Mazes', 'Ties', 'Wins', 'Win%']
                    ])
                    print ' | '.join([
                        str(x).ljust(8) for x in
                        [mapid, points, moves, mazes, ties, wins, (wins / (mazes + 0.0))]
                    ])
        print ' | '.join([
            x.ljust(8) for x in ['Mapid', 'Points', 'Moves', 'Mazes', 'Ties', 'Wins']
        ])
        print ' | '.join([
            str(x).ljust(8) for x in [mapid, points, moves, mazes, ties, wins]
        ])
        if options['reverse']:
            mapid -= 1
        else:
...
def get_maptype_history(maptype, options={}):
    ...
    while mapid >= 0:
        if get_map_type(mapid) == maptype:
            ...
            print ' | '.join([
                str(x).rjust(y[1]) for (x, y) in zip([mazes, mapid, maxscore, nties, time], headers)
            ]) + ' | ' \
...
def get_fastest_ever():
    ...
    while mapid >= 0:
        ...
        if (time is not None) and (best is None or best[1] > time):
...
def graph_win_times(options={}):
    ...
    times = [[None for _ in range(options['ndays'])] for i in range(0, 3)]
    day = 0
